//Universidad del Desarrolo - Facultad de Diseño - Diseño Digital - Expresión Digital II
...
//de input de mouse (el click cambia el color del que ahora podemos llamar cursor a un celeste), 
//y de teclado (con la tecla 't', se agranda el tamaño de los cuadrados que componen la grilla, y con cualquier otra tecla, los cuadrados vuelven a su tamaño original). 
//Al mismo tiempo, el programa al no recibir input, este continúa teniendo un cambio cíclico de background.


int tam = 20; //Declaramos el valor de tamaño 
int valor = color(0, 200, 170); //Declaramos el color correspondiente  a la variable 'valor'
...
void draw(){ //A continuación se 'dibuja' en el canvas:
  for (int i = 0; i < 21; i++) {//Se declara i=0, y que este vaya aumentando en 1 por cada frame hasta llegar a 20
    for (int j = 0; j < 21; j++) {//Se declara j=0, y que este vaya aumentando en 1 por cada frame hasta llegar a 20
      ...
      if((mouseX > x - tam && mouseX < x + tam) && (mouseY > y - tam && mouseY < y + tam)){//En el caso de que la posición del mouse en 'x' sea mayor que el valor de 'x' menos 20, y además, menor que el velor de x más 20
                                                                                           //Y se cumpla también que la posición del mouse en 'y' sea mayor que el valor de 'x' menos 20, y además, menor que el valor de 'y' más 20
      ...
      stroke(0);//La figura que se dibuja a continuación tiene un grosor de cero
...
void keyPressed(){//En el caso de oprimir una tecla:
  if(key == 't'){//Si se oprime la tecla 't'
    tam += 20;//El tamaño de los cuadrados aumentará en 20, osea 40, por lo que caben la mitad de los cuadrados que cabian anteriormente en el canvas 
  }else{//Por otor lado, en el caso de oprimir cualquier tecla que no sea la 't'
    tam = 20;//el tamaño delos cuadrados vuelven a su valor original, o sea, 20
  }
}
void mousePressed(){//En caso de oprimir cualquier botón del mouse:
 if (valor == color(0, 200, 170)) {//El color verdoso del cuadrado/puntero 
    valor = color(0, 170, 200);//cambia a celeste
  } else {//en caso de volver a oprimir cualquir botón del mouse
    valor = color(0, 200, 170);//Este vuelve a su color verdodo original
  }
}