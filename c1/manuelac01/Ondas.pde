...
class Ondas {
  ...
  /*La función "lineas", creara lineas en paralelo, en el alto y bajo del canvas,
   con un movimiento ondulado.
   Este movimiento cambia la altura de la ondulación, y el ancho de las lineas.
   */
    ...
    for (int i=0; i<100; i++) { /*/i es igual a 0, y aumenta en 1, hasta llegar a 100 
     (cantidad de lineas creadas).*/
      float fin = map(sin(vel+i*2), -5, 1, 450, 100); /*Se crea la onda y el 
       movimiento de esta.*/
      line(i*10, height, i*10, height-fin); //Lineas inferior.
