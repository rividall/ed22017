...
void setup() {
  ...
  for (int i = 0; i < tabla.getRowCount(); i++) {
    ...
    int maxt = row.getInt("Max T");
    int mint = row.getInt("Min T");
    int meant = row.getInt("Mean T");
    int meandp = row.getInt("Mean DP");
    int meanh = row.getInt("Mean H");
    int meanv = row.getInt("Mean V");

    e[i] = new Iquitos(fecha, maxt, mint, meant, meandp, meanh, meanv);
  }
}
